...
# NN model type to fine-tune (must be either 'eegnet_hilb' or 'eegnet')
model_type_finetune = 'eegnet_hilb'
layers_to_finetune = ['all', ['conv2d', 'batch_normalization'],
                      ['batch_normalization', 'depthwise_conv2d',
...
# if True, use percentage values (otherwise, use number of trials)
use_per_vals = True
...
for i in range(max_train_parts):
    ...
    run_nn_models(sp_curr, n_folds_same, combined_sbjs, ecog_lp, ecog_roi_proj_lp, test_day='last', do_log=False,
                  ...
                  kernLength=hyps_same['kernLength'], kernLength_sep=hyps_same[
                      'kernLength_sep'], dropoutType=hyps_same['dropoutType'],
                  D=hyps_same['D'], F2=hyps_same['F2'], n_estimators=hyps_same['n_estimators'], max_depth=hyps_same['max_depth'])
