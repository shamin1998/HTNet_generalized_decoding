...
def htnet(nb_classes, Chans=64, Samples=128,
    ...
    if useHilbert:
        ...
        if compute_val == 'relative_power':
            ...
            X1 = Lambda(apply_hilbert_tf, arguments={
                        'do_log': True, 'compute_val': 'power'})(block1)

            # Subtract off baseline (at beginning of input data trials)
            # average across all time points
            X2 = AveragePooling2D((1, X1.shape[-1]//base_split))(X1)
            X2 = Lambda(lambda x: tf.tile(x[..., :1], tf.constant(
                [1, 1, 1, Samples], dtype=tf.int32)))(X2)
